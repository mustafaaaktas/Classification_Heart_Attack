...
def plot_column_distribution(data, col1, col2):
    """
    Function to plot up to four different visualizations for
        two columns in the given dataset.
    ...
    plt.suptitle(f'{col1} vs {col2} Distribution',
                 fontsize=18,
                 fontweight='bold',
                 color='navy')
    ...
    # Plot the fourth (optional)
    # Scatter plot to see if there's any visible pattern for binary class
    ...
    plt.subplots_adjust(top=0.9)  # Adjust to fit the subtitle
    plt.show()
